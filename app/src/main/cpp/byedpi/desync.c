        ...
        struct eval *val, struct buffer *buff, ssize_t *np, bool *wait)
{
    ...
    ssize_t offset = (ssize_t)buff->offset;
    ...
    for (; r > 0 || i < dp.parts_n; r--) {
        ...
        switch (part.m) {
            ...
            case DESYNC_DISOOB:
                if (is_first_of_pair && setttl(sfd, 1) < 0) {
                    s = -1;
                    break;
                }
                val->restore_ttl = 1;
        ...
        if (!is_first_of_pair) val->pair->part_sent = curr_part;
        ...
        if (sock_has_notsent(sfd) 
            ...
            set_timer(pool, val, params.await_int);
            *wait = true;
            return (is_first_of_pair ? lp : pos) - offset;
        }
        restore_state(val);
        
        if (!is_first_of_pair) lp = pos;
    }
    // send all/rest
    if (lp < n) {
