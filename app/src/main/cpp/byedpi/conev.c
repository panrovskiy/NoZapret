...
void del_event(struct poolhd *pool, struct eval *val) 
{
    ...
    // IMPORTANT: Check server_fd to avoid closing tun_fd or other external descriptors
    extern int server_fd;
    if (val->fd != server_fd && val->fd > 0) {
        close(val->fd);
    }
...
struct eval *next_event(struct poolhd *pool, int *offs, int *type, int ms)
{
    for (int i = *offs; ; i--) {
        ...
        short rtype = pool->pevents[i].revents;
        if (!rtype) {
            continue;
        }
